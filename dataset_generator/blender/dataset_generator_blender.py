"""Gerador de dataset COCO-pose sintético via Blender headless.
...
_parser.add_argument("--start", type=int, default=0,
                     help="Índice inicial das imagens (default 0)")
_parser.add_argument("--num",   type=int, default=None,
                     help="Quantas amostras gerar (default: do YAML)")
...
# Seed diferente para cada bloco → poses/câmeras não se repetem
rng  = make_rng(dcfg.seed + START_INDEX)
rand = DomainRandomizer(rng, rcfg)

print(f"[GEN] start={START_INDEX}  num={NUM_SAMPLES}  seed={dcfg.seed + START_INDEX}")

W, H        = 640, 480
SENSOR_W_MM = 36.0
LENS_MM     = (SENSOR_W_MM / 2.0) / math.tan(math.radians(60.0 / 2.0))
OUT_ROOT    = PROJ_ROOT / dcfg.output_dir
...
# Carrega JSONs existentes para continuar (append) em vez de sobrescrever
ann_dir = OUT_ROOT / "annotations"
split_builders: dict[str, CocoDatasetBuilder] = {}
for s in dcfg.splits:
    b = CocoDatasetBuilder(f"NAO synthetic COCO-pose (Blender) — {s}")
    existing = ann_dir / f"person_keypoints_{s}.json"
    b.load_existing(existing)
    split_builders[s] = b
    if b.images:
        print(f"[GEN] {s}: carregou {len(b.images)} amostras existentes")
...
if arm is None:
    sys.exit("[GEN] NAO_Armature não encontrado. Rode com nao_full.blend.")

poser = NaoPoserBlender(arm, PROJ_ROOT / "protos" / "nao_exported.urdf")
...
def _apply_dof(cam_pos):
    focus_dist = float(np.linalg.norm(np.array(cam_pos)))
...
for i in range(NUM_SAMPLES):
    ...
    print(f"[GEN] {i+1}/{NUM_SAMPLES}  #{frame_idx}  {split}  vis={n_vis}/17 occ={n_occ}/17  {img_name}")
...
for split, builder in split_builders.items():
    ...
    print(f"[GEN] {split}: {len(builder.images)} imagens → {out_json}")
